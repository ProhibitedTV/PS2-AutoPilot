...
def _format_input(row: dict[str, Any]) -> str:
    ...
    if x is not None or y is not None:
        return f"[{stamp}] D{decision!s:<6} INPUT {kind:<10} x={float(x or 0):+.2f} y={float(y or 0):+.2f}"
...
def log_main() -> None:
    ...
    parser.add_argument("--inputs", action="store_true", help="Tail controller input log instead")
    args = parser.parse_args()

    root = Path(args.root)
    path = root / ("input.jsonl" if args.inputs else "events.jsonl")
    formatter = _format_input if args.inputs else _format_event

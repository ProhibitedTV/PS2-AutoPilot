...
def test_registry_selects_guitar_hero_v4_and_aliases():
    profile = build_profile({"name": "gh1", "difficulty": "easy"})
    assert type(profile) is GuitarHeroV4Profile
    assert GuitarHeroProfile is GuitarHeroV4Profile
    assert GuitarHeroV2Profile is not GuitarHeroV4Profile
    assert GuitarHeroV3Profile is not GuitarHeroV4Profile

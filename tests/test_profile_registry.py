...
def test_registry_selects_exact_active_madden_v25_factory():
    profile = build_profile({"name": "madden2005", "ocr_enabled": False})
    assert type(profile) is Madden2005V25Profile
...
def test_registry_selects_exact_active_jak_v22_without_reusing_madden():
    ...
    assert not isinstance(profile, Madden2005V25Profile)
